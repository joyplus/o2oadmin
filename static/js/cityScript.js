var citySelector = {};
cityList = new Array();
cityList.removeCity = function(city) {
	var index = this.indexOf(city);
	if (index > -1) {
		this.splice(index, 1);
	}
...
citySelector.pc[2] = new Array("内蒙", "呼和浩特|包头|乌海|赤峰|呼伦贝尔|通辽|乌兰察布|鄂尔多斯|巴彦淖尔");
citySelector.pc[3] = new Array("辽宁", "沈阳|大连|鞍山|抚顺|本溪|丹东|锦州|营口|阜新|辽阳|盘锦|铁岭|朝阳|葫芦岛");
citySelector.pc[4] = new Array("吉林", "长春|吉林|四平|辽源|通化|白山|松原|白城");
citySelector.pc[5] = new Array("黑龙江", "哈尔滨|齐齐哈尔|牡丹江|佳木斯|大庆|绥化|鹤岗|鸡西|黑河|双鸭山|伊春|七台河");
...
citySelector.pc[8] = new Array("安徽", "合肥|芜湖|蚌埠|马鞍山|淮北|铜陵|安庆|黄山|滁州|宿州|池州|淮南|巢湖|阜阳|六安|宣城|亳州");
citySelector.pc[9] = new Array("福建", "福州|厦门|莆田|三明|泉州|漳州|南平|龙岩|宁德");
citySelector.pc[10] = new Array("江西", "南昌市|景德镇|九江|鹰潭|萍乡|新馀|赣州|吉安|宜春|抚州|上饶");
...
citySelector.pc[13] = new Array("湖北", "武汉|宜昌|荆州|黄石|荆门|黄冈|十堰|随州|咸宁|孝感|鄂州|襄阳");
citySelector.pc[14] = new Array("湖南", "长沙|常德|株洲|湘潭|衡阳|岳阳|邵阳|益阳|娄底|怀化|郴州|永州|张家界");
...
citySelector.pc[17] = new Array("海南", "海口|三亚|三沙");
citySelector.pc[18] = new Array("四川", "成都|绵阳|自贡|攀枝花|泸州|德阳|广元|遂宁|内江|乐山|资阳|宜宾|南充|达川|雅安|巴中|广安|眉山");
citySelector.pc[19] = new Array("贵州", "贵阳|六盘水|遵义|安顺|铜仁|毕节");
citySelector.pc[20] = new Array("云南", "昆明|昭通|曲靖|玉溪|普洱|保山|丽江|临沧");
citySelector.pc[21] = new Array("西藏", "拉萨");
citySelector.pc[22] = new Array("陕西", "西安|铜川|宝鸡|咸阳|渭南|汉中|安康|商洛|延安|榆林");
citySelector.pc[23] = new Array("甘肃", "兰州|嘉峪关|金昌|白银|天水|酒泉|张掖|武威|定西|陇南|平凉|庆阳");
citySelector.pc[24] = new Array("宁夏", "银川|石嘴山|吴忠|固原|中卫");
citySelector.pc[25] = new Array("青海", "西宁|海东");
citySelector.pc[26] = new Array("新疆", "乌鲁木齐|克拉玛依");
citySelector.pc[27] = new Array("其它", "澳门|香港|台湾");

citySelector.hotCity = ["北京", "上海", "重庆", "天津"];
...
function searchcity(str) {
	...
	var result = "";
	for (index = 0; index < citySelector.pc.length; index ++) {
		var cities = citySelector.pc[index][1].split("|");
		var province = citySelector.pc[index][0];
		for (i = 0; i < cities.length; i ++) {
			if (contains(cities[i], str, true)) {
				result += province + "-" + cities[i] + ";";
			}	
		}
	}
	for (i = 0; i < citySelector.hotCity.length; i ++) {
		if (contains(citySelector.hotCity[i], str, true)) {
			result += citySelector.hotCity[i] + ";";
		}
	}
	if (result != "") {
		alert(result);
...
citySelector.cityInit = function (input) {
    ...
    $("#" + input).click(function () {
        ...
        $("#js_cityBox").on("click", ".js_cityName", function (e) {
            e.stopPropagation();
			var index = cityList.indexOf($(this).html());
			if (index > -1) {
				// the city already selected
				$("#js_cityBox").remove();
				return;
			}
            //$("#" + _input).val($(this).html());
            $("#js_cityBox").remove();
            var appendtext = '<li data-id="12" class="selected">'+$(this).html()+'<span style="display:none;" class="get_html">'+$(this).html()+'</span>'+'<i class="selected-i" onclick="onDeleteCity(this)">×</i></li>'
