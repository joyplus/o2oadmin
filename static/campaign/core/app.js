define(function(require, exports){
    ...
    exports.getUserCampany = function(){
        if(user_data && user_data.campany){
            return user_data.campany;
        }else{
            return {};
        }
    }
    ...
    exports.getUserId = function(){
        if(user_data && user_data.campany && user_data.campany.UserId){
            return user_data.campany.UserId;
        }else{
            return '';
        }
    }
    // 获取用户的创建者userid
    exports.getCreateUserId = function(){
        if(user_data && user_data.campany && user_data.campany.CreateUserId){
            return user_data.campany.CreateUserId;
        }else{
            return '';
        }
    }
    ...
    exports.getUserType = function(){
        if(user_data && user_data.campany && user_data.campany.Type){
            return user_data.campany.Type;
        }else{
            return '';
        }
    }
    // 获取用户的员工的userid
    exports.getEmployeeUserId = function(){
    ...
    // 获取来源的公司账号userid
    exports.getOriginUserId = function(){
        if(user_data && user_data.origin_campany && user_data.origin_campany.UserId){
            return user_data.origin_campany.UserId;
        }else{
            return '';
        }
    }
    // 获取账号权限，参数auth_arr为权限点数组
    exports.getUserAuth = function(auth_arr, isEmployee){
        ...
        if(isEmployee){
            ...
            if(user_data && user_data.campany && user_data.campany.UserId){
                userId = user_data.campany.UserId;
            }
        }

        if(auth_arr && auth_arr.length){
            util.each(auth_arr, function(item){
                if(item && item == userId){
                    result = true;
                }
            })
    ...
    DataCenter.prototype = {
        ...
        execute: function(request){
            ...
            if (uri.length > 2048){
                uri = this.resolve(request[dcURI]);
                request[dcData] = $.extend(request[dcData], request[dcParam]);
                request[dcData].decode = 1;
                request[dcMode] = 'POST';
            }
            request[dcAjax] = $.ajax({
                type: request[dcMode],
        ...
        resolve: function(uri, param){
            ...
            for (var i=0; i<list.length; i++){
                ...
                if (uri.indexOf(prefix+'/') === 0 || uri.indexOf(prefix+'?') === 0){
                    uri = target + uri.substr(prefix.length);
                    break;
                }
            }
            // 合并param参数到URI中
    ...
    function Language(cb){
        ...
        window.LANG = this.LANG = function(text){
            if (me.translate && me.translate.hasOwnProperty(text)){
                ...
                var transFunc = me.translate && me.translate.func;
                if ( !me.isDefault() && transFunc ) {
        ...
        this.isDefault = function() {
            return this.load_name === default_name;
        }
        ...
        this.load = function(){
            ...
            ajaxId = datacenter.get('/main/i18n/'+this.load_name+'/translate.json', onLoad);
        }
        function onLoad(err, data){
            ...
            var path = me.translate && me.translate._path_;
            var func = me.translate && me.translate._func_;
            if (path && func) {
                require.async(path, function(m) {
                    if (m && m[func]) {
                        me.translate.func = m[func]
                        callback(false);
                    } else {
                        callback(true);
                    }
    ...
    exports.notify = function(message, title, type){
        var core = this.core;
        if (!core){
            return false;
        }
        ...
        var win = core.get('SYSTEM_NOTIFY');
        if (!win){
            core.createAsync('SYSTEM_NOTIFY', 'popwin_base.notify', null, function(win){
                win.setData(data).show();
            });
        }else {
            win.setData(data).show();
        }
    ...
    var localStore;
    try {
        localStore = window.localStorage || new UserData();
    }catch(e){
        localStore = new UserData();
    }
    ...
    exports.alert = function(text, callback, alertType,buttons){
        if (!alertType){
            alertType = 'alert';
        }
        ...
        if(alertQueue.length == 1){
            ...
            var win = core.get('SYSTEM_ALERT');
            if (!win){
                core.createAsync('SYSTEM_ALERT', 'popwin_base.alertPopwin', {'data': c});
            }else {
                win.setData(c).show();
            }
        }
    }
    exports.confirm = function(text, callback, buttons){
        this.alert(text, callback, 'confirm',buttons);
    }

    var loading_mask = null;
    var loading_stat = 0;
    exports.showLoading = function(){
        loading_stat++;
        if (!loading_mask){
            this.core.createAsync(
                'loading', 'common.loadingMask',
                {target: 'body', auto_show: false},
                function(mod){
                    loading_mask = mod;
                    if (loading_stat > 0){
                        mod.show();
    ...
    exports.hideLoading = function(){
        ...
        if (loading_mask && loading_stat <= 0){
            loading_mask.hide();
        }
    }
});
