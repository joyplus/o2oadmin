...
func init() {
	orm.RegisterModel(new(LtvApp))
}

// AddLtvApp insert a new LtvApp into database and returns
...
// GetLtvAppById retrieves LtvApp by Id. Returns error if
// Id doesn't exist
...
// GetAllLtvApp retrieves all LtvApp matches certain condition. Returns empty list if
// no records exist
func GetAllLtvApp(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(LtvApp))
	// query k=v
	for k, v := range query {
...
// UpdateLtvApp updates LtvApp by Id and returns error if
...
// DeleteLtvApp deletes LtvApp by Id and returns error if
...
func DeleteLtvApp(id int) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&LtvApp{Id: id}); err == nil {
